...
public class Taller8 {
    ...
    */
    public static int polaca (String string) throws EmptyStackException{

        Stack<Integer> p = new Stack<Integer>();
        String operacion[] = string.split(" ");

        for (String some : operacion) {

            String operators = "+/-*";

            if (operators.contains(some)) {
                int last = p.pop();
                int penultimate = p.pop();

                if (some.equals("+")) {
                    p.push(last + penultimate);
                }
                if (some.equals("-")) {
                    p.push(penultimate - last);
                }
                if(some.equals("*")){
                    p.push(penultimate * last);
                }
            }else {
                p.push(Integer.parseInt(some));
            }
    ...
    public static void main(String[] args) {
        ...
        String test = "40 5 * 5 3 + +";

        System.out.println(polaca(test));
